...
不要把配音稿改写成摘要，也不要漏掉。配音稿只用于驱动人声、口型和节奏，不是画面元素；不得生成字幕、提词器、标题条、说明卡、气泡文字、屏幕字或任何可读文字。
...
10. 无文字硬约束：最终视频画面必须是纯净无字画面；任意一帧里都不能出现中文、英文、数字、乱码、假字、字幕、水印、标题、UI文字、屏幕字、白板字、包装可读文字或类似字幕的文本痕迹；只要出现任何文字痕迹，本次视频视为失败，需要重新生成
...
画质模糊，画面闪烁，抖动感，帧漂移，主体变形，解剖错误，面部不一致，主体消失，视觉噪点，低质量，水印，文字叠加，字幕，双语字幕，歌词字幕，旁白字幕，标题栏，下三分之一标题，按钮文案，弹幕，CTA文字，可读文字，中文文字，英文文字，数字编号，时间码，乱码文字，假字，UI文字，屏幕字，白板字，海报字，瓶身可读小字，包装可读文字
