...
DEFAULT_TTS_TEXT = (
    "如果你是新手，我建议先从低浓度、低频率开始。"
    "比如一周两次，晚上用，先用豌豆大小，不要全脸猛糊。"
    "保湿和防晒必须跟上，否则好成分也可能变成负担。"
)
...
图片2是“1分钟讲清视黄醇”的无头 Production Board 大图，不是最终视频画面。图片2包含 4 个 15 秒段落，这次只读取故事板中编号 3 的区域，也就是中下部第三列蓝框区域：`3 / 30-45s / 新手怎么开始`。不要读取编号 1、2、4 的故事内容来生成本段动作或台词。
图片2只用于读取护肤科普工作室场景、桌面产品、视黄醇瓶身、新手使用频率卡片、保湿和防晒提示图标、暖灰色灯光、镜头语言和 Segment 3 的内容计划。图片2里的人脸是无五官占位脸，不要采用图片2的人脸；真人脸、发型、眼镜、年龄感、衣着和五官必须始终以图片1为准。
音频1是当前 Segment 3 的真实 TTS 口播音频，人物口型、停顿、表情和镜头节奏要尽量贴合音频1。

不要生成 production board 页面，不要生成网格排版，不要生成顶部标题栏，不要生成分区说明文字，不要生成故事板卡片，不要生成任何可见字幕或说明文字。

本次只生成 Segment 3 / 30-45 秒：新手怎么开始。
画面必须是一个真实口播视频：图片1中的男性讲师坐在图片2规划的护肤科普工作室桌前，面对镜头自然讲解新手如何开始使用视黄醇。保持图片1的人物身份和衣着质感，保持图片2的暖灰色空间、桌面护肤品、视黄醇瓶身、使用频率卡片、保湿防晒提示图标、柔和主光和干净商业质感。

镜头节奏：
30-33 秒：中近景，主角手持或指向视黄醇瓶身，语气耐心，提醒新手不要急着高频使用。
33-38 秒：切到产品 close-up 或使用频率卡片，画面出现“一周两次、晚上使用、豌豆大小”的视觉提示感，但不要生成可读大字 UI。
38-42 秒：回到主角中近景，主角用手势解释保湿和防晒必须跟上，表情认真但不吓人。
42-45 秒：主角 close-up，总结低浓度、低频率、慢慢建立耐受，语气温和、可信、像专业顾问。

口播台词必须以音频1为准：
「{tts_text}」

禁止：
不要使用图片2的无五官占位脸替代图片1真人脸，不要换脸，不要换发型，不要换眼镜，不要换服装，不要出现第二个人，不要跳到新场景，不要夸张表演，不要医疗承诺，不要说一夜变年轻，不要读取 Segment 1、2、4 的故事板内容，不要把图片2中的文字和排版复刻到视频里。
""".strip()
